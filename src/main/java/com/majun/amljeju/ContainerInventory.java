...
public class ContainerInventory extends Container {
    ...
    public ContainerInventory(InventoryPlayer inventoryPlayer, final EntityPlayer player) {
        this.player = player;
        this.craftMatrix = new InventoryCrafting(this, 2, 2);
        this.craftResult = new InventoryCraftResult();

        addSlotToContainer((Slot)new SlotCrafting(player, this.craftMatrix, (IInventory)this.craftResult, 0, 187, 26));  //제작된 아이템 위치 설정
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++)
                addSlotToContainer(new Slot((IInventory)this.craftMatrix, j + i * 2, 128 + j * 20, 16 + i * 19));  //2X2 조합 위치 선정
        }
    ...
    public void onContainerClosed(EntityPlayer playerIn) {
        ...
        if (!playerIn.world.isRemote)
            clearContainer(playerIn, playerIn.world, (IInventory)this.craftMatrix);
    }
